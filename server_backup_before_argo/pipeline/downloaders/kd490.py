"""
Kd490 (Diffuse Attenuation Coefficient) Downloader

Downloads L4 gap-free Kd490 data from Copernicus Marine.
"""
...
class Kd490Downloader(BaseDownloader):
    """Downloader for Diffuse Attenuation Coefficient at 490nm (L4 gap-free)."""
    ...
    def _convert(self, nc_path: Path, cog_path: Path, date: str) -> dict[str, Any] | None:
        """Convert Kd490 NetCDF to COG."""
        try:
            ds = load_netcdf(nc_path, variables=["KD490"])
            
            # Find KD490 variable (case insensitive)
            kd_var = None
            ...
            if kd_var is None:
                logger.error(f"KD490 variable not found. Available: {list(ds.data_vars)}")
                return None
            
            kd490 = ds[kd_var]
            if "time" in kd490.dims:
                kd490 = kd490.isel(time=0)
            
            kd_data = kd490.values.astype(np.float32)
            ...
            if lat[0] < lat[-1]:
                kd_data = np.flipud(kd_data)
                lat = lat[::-1]
            
            # Compute stats
            stats = compute_stats(kd_data)
            
            # Keep as float32 (log-scale visualization common)
            # NaN for nodata
            ...
            ds.close()
            logger.info(f"Created Kd490 COG: {cog_path}")
            return stats
            
        except Exception as e:
            logger.error(f"Kd490 conversion error: {e}")
            import traceback
            traceback.print_exc()
            return None
