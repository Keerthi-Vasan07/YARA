...
import { DatasetInfo, FilterMode, GradientStop } from '../types/dataset';
import { getLocalFrameUrl, fetchLocalGrid } from '../services/localDatasetApi';
...
export const CesiumViewer = forwardRef<CesiumViewerHandle, CesiumViewerProps>(
  ...
  const updateDataLayers = useCallback(async (date: string, layers: string[]) => {
    ...
    if (activeMode === 'local' && activeDataset && viewer && !viewer.isDestroyed()) {
      ...
      const requestKey = `local:${activeDataset.id}:${varToRender}:${localTimeIndex}:${colorScaleMin}:${colorScaleMax}:${colormap || ''}:${analysisMode}:${exactTemperature}:${exactTolerance}:${rangeMin}:${rangeMax}:${JSON.stringify(gradientStops || [])}`;
      ...
      const imageUrl = getLocalFrameUrl(
        activeDataset.id, varToRender, localTimeIndex, undefined, colormap,
        colorScaleMin, colorScaleMax, analysisMode,
        exactTemperature ?? undefined, exactTolerance,
        rangeMin ?? undefined, rangeMax ?? undefined, gradientStops
      );
  ...
  }, [raiseLabelsToTop, activeMode, activeDataset, localVariable, localTimeIndex, colorScaleMin, colorScaleMax, colormap, analysisMode, exactTemperature, exactTolerance, rangeMin, rangeMax, gradientStops]);
  ...
  useEffect(() => {
    ...
    if (activeMode === 'local') {
      const localKey = `${activeDataset?.id}:${localVariable}:${localTimeIndex}:${colorScaleMin}:${colorScaleMax}:${colormap}:${analysisMode}:${exactTemperature}:${exactTolerance}:${rangeMin}:${rangeMax}:${JSON.stringify(gradientStops || [])}`;
    ...
    updateDataLayers(selectedDate, visibleLayers);
  }, [selectedDate, isLoading, updateDataLayers, visibleLayers, activeMode, activeDataset, localVariable, localTimeIndex, colorScaleMin, colorScaleMax, colormap, analysisMode, exactTemperature, exactTolerance, rangeMin, rangeMax, gradientStops]);

  // 1-degree (or native-resolution) reference grid overlay for local datasets.
  // Rendered as viewer.entities polylines (not an imagery layer) so it draws above
  // imagery automatically; labels are explicitly re-raised so they stay above the grid.
  ...
  useEffect(() => {
    ...
    for (const entity of gridEntitiesRef.current) {
      viewer.entities.remove(entity);
    }
    gridEntitiesRef.current = [];

    if (!gridEnabled || activeMode !== 'local' || !activeDataset) {
      viewer.scene.requestRender();
      return;
    }

    let cancelled = false;
    fetchLocalGrid(activeDataset.id)
      .then((grid) => {
        if (cancelled) return;
        ...
        for (const lat of grid.lat_lines) {
          newEntities.push(
            v.entities.add({
              polyline: {
                positions: Cesium.Cartesian3.fromDegreesArray([ext.west, lat, ext.east, lat]),
                width: 1,
        ...
        for (const lon of grid.lon_lines) {
          newEntities.push(
            v.entities.add({
              polyline: {
                positions: Cesium.Cartesian3.fromDegreesArray([lon, ext.south, lon, ext.north]),
                width: 1,
                material: Cesium.Color.WHITE.withAlpha(0.25),
              },
            })
        ...
        gridEntitiesRef.current = newEntities;
        raiseLabelsToTop();
        v.scene.requestRender();
      })
      .catch((err) => {
        console.error('[YARA Grid] Failed to load grid lines:', err);
      });

    return () => {
      cancelled = true;
    };
  }, [gridEnabled, activeMode, activeDataset, raiseLabelsToTop]);
