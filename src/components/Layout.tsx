...
import { DatasetInfo, FilterMode, GradientStop, LocalPointQueryResponse } from '../types/dataset';
import { fetchActiveMode, fetchLocalPoint, fetchLocalFrameStats } from '../services/localDatasetApi';
...
export function Layout() {
  ...
  // Local-dataset temperature analysis state (exact/range filtering + custom gradient + grid)
  const [analysisMode, setAnalysisMode] = useState<FilterMode>('none');
  const [exactTemperature, setExactTemperature] = useState<number | null>(null);
  const [exactTolerance, setExactTolerance] = useState<number>(0.5);
  ...
  const [matchingCellsInfo, setMatchingCellsInfo] = useState<{ matching: number; total: number } | null>(null);
  const [localPointData, setLocalPointData] = useState<LocalPointQueryResponse | null>(null);
  const [localPointLoading, setLocalPointLoading] = useState(false);

  // Debounce local-dataset render params so slider/gradient-editor drags don't fire
  // a network request per tick; the color-bar/gradient-editor preview stays live.
  const liveLocalRenderParams = useMemo(() => ({
    colorScaleMin, colorScaleMax, colormap, analysisMode, exactTemperature,
    exactTolerance, rangeMin, rangeMax, gradientStops,
  }), [colorScaleMin, colorScaleMax, colormap, analysisMode, exactTemperature, exactTolerance, rangeMin, rangeMax, gradientStops]);
  const debouncedLocalRenderParams = useDebouncedValue(liveLocalRenderParams, 300);
  ...
  const handleCloseLocalPointPanel = useCallback(() => {
    setLocalPointData(null);
    setLocalPointLoading(false);
    setClickedPosition(null);
    setScreenPosition(null);
  ...
  // Fetch match-count stats for the active exact/range filter, driving
  // "N matching cells" / "No dataset values found" messaging in ColorScaleControls.
  useEffect(() => {
    if (activeMode !== 'local' || !activeDataset || debouncedLocalRenderParams.analysisMode === 'none') {
      setMatchingCellsInfo(null);
      return;
    }
    const { analysisMode: mode, exactTemperature: exactVal, exactTolerance: tol, rangeMin: rMin, rangeMax: rMax } = debouncedLocalRenderParams;
    if (mode === 'exact' && exactVal === null) {
      setMatchingCellsInfo(null);
      return;
    }
    if (mode === 'range' && (rMin === null || rMax === null || rMin > rMax)) {
      setMatchingCellsInfo(null);
      return;
    }
    let cancelled = false;
    fetchLocalFrameStats(
      activeDataset.id,
      localVariable || activeDataset.default_variable,
      localTimeIndex,
      mode,
      ...
      .then((stats) => {
        if (!cancelled) setMatchingCellsInfo({ matching: stats.matching_cells, total: stats.total_cells });
      })
      .catch(() => {
        if (!cancelled) setMatchingCellsInfo(null);
      });
    return () => {
      cancelled = true;
    };
  }, [activeMode, activeDataset, localVariable, localTimeIndex, debouncedLocalRenderParams]);
  ...
  return (
    <Box sx={{ 
      ...
      <CesiumViewer 
        ...
        colorScaleMin={debouncedLocalRenderParams.colorScaleMin}
        colorScaleMax={debouncedLocalRenderParams.colorScaleMax}
        drawingMode={false}
        onBboxDrawn={() => {}}
        ...
        colormap={debouncedLocalRenderParams.colormap}
        analysisMode={debouncedLocalRenderParams.analysisMode}
        exactTemperature={debouncedLocalRenderParams.exactTemperature}
        exactTolerance={debouncedLocalRenderParams.exactTolerance}
        rangeMin={debouncedLocalRenderParams.rangeMin}
        rangeMax={debouncedLocalRenderParams.rangeMax}
        gradientStops={debouncedLocalRenderParams.gradientStops}
      ...
      {colorScaleOpen && (
        <ColorScaleControls
          ...
          isLocalDataset={activeMode === 'local' && !!activeDataset}
          analysisMode={analysisMode}
          onAnalysisModeChange={setAnalysisMode}
          exactTemperature={exactTemperature}
          onExactTemperatureChange={setExactTemperature}
          ...
          matchingCellsInfo={matchingCellsInfo}
          gridEnabled={gridEnabled}
      ...
      {/* Local Dataset Point Query Panel */}
      <LocalPointInfoPanel
        data={localPointData}
        loading={localPointLoading}
        onClose={handleCloseLocalPointPanel}
        screenPosition={screenPosition}
      />
